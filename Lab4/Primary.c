...
void primary(int sockfd, double ber) {
  ...
  char msg[DATA_LENGTH], srvReply[150];
	...
	struct packet_buffer buff[13];
  struct packet_buffer frame[13];
  ...
  while(1){
    ...
    // fgets(msg, DATA_LENGTH+1, stdin);
      ...
      buildPacket(frame[i].packet, DATA_PACKET, msg, packetNum);
		  //Introduce error
      IntroduceError(frame[i].packet, ber);
	  }
	  
	  while(1){
      //Sends the packets of corrupted data
      if(!flag){		    
        for(i = 0; i < 3; i++){
			   
          if( send(sockfd , frame[win[i]].packet, PACKET_SIZE, 0) < 0){
				    perror("Send failed");
          }
      ...
      //during retransmission, we do not want to 
      //retransmitt corrupted packets. the seperate
      //loop sends buffered good data that is sure to not error
      else{
        for(i = 0; i < 3; i++){
